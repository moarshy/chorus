...
export function AgentChatPanel() {
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 'n' && workspace && agent) {
        e.preventDefault()
    ...
    [workspace, agent, createConversation, isThisConversationStreaming, stopAgent]
  )
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <ConversationSelector workspaceId={workspace.id} agentId={agent.id} />

      {/* Claude CLI Not Installed Warning */}
      {isClaudeChecked && !claudePath && (
        <div className="mx-2 mt-2 px-3 py-2 bg-yellow-900/30 border border-yellow-500/50 rounded-lg flex items-start gap-2">
      ...
      {/* Error Banner */}
      {error && (
        <div className="mx-2 mt-2 px-3 py-2 bg-red-900/30 border border-red-500/50 rounded-lg flex items-center gap-2">
